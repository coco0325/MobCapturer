...
public class ParrotAdapter extends AbstractTameableAdapter<Parrot> {
	...
	public List<String> getLore(JsonObject json) {
		...
		lore.add(ChatColor.GRAY + "種類: " + ChatColor.WHITE + StringUtils.format(json.get("variant").getAsString()));
		
		if (!json.get("ownerUUID").isJsonNull()) {
			lore.add(ChatColor.GRAY + "坐下狀態: " + ChatColor.WHITE + json.get("sitting").getAsBoolean());
		}
